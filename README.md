# eve-examples

Open-source Eve agent templates. Each template is maintained as a standalone
upstream repository and imported here as a detached subdirectory.

| Template | Directory | Upstream | Imported commit |
| --- | --- | --- | --- |
| Personal Agent | [`personal-agent-template`](./personal-agent-template) | [vercel-labs/personal-agent-template](https://github.com/vercel-labs/personal-agent-template) | `ec986e5` |
| Slack Agent | [`eve-slack-agent-template`](./eve-slack-agent-template) | [vercel-labs/eve-slack-agent-template](https://github.com/vercel-labs/eve-slack-agent-template) | `bb35e1f` |
| Content Agent | [`eve-content-agent-template`](./eve-content-agent-template) | [vercel-labs/eve-content-agent-template](https://github.com/vercel-labs/eve-content-agent-template) | `dd42d0a` |
| Sanity Copilot | [`sanity-copilot-eve-template`](./sanity-copilot-eve-template) | [vercel-labs/sanity-copilot-eve-template](https://github.com/vercel-labs/sanity-copilot-eve-template) | `f076b80` |
| Chat Template | [`eve-chat-template`](./eve-chat-template) | [vercel-labs/eve-chat-template](https://github.com/vercel-labs/eve-chat-template) | `4792ac1` |
| Typefully Agent | [`typefully-eve-template`](./typefully-eve-template) | [vercel-labs/typefully-eve-template](https://github.com/vercel-labs/typefully-eve-template) | `3627b22` |
| LLM Council | [`eve-llm-council-template`](./eve-llm-council-template) | [vercel-labs/eve-llm-council](https://github.com/vercel-labs/eve-llm-council) | `b591cd7` |
